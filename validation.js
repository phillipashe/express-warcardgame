...
request('PUT', '/game', function(err, res) {
  ...
  function checkTurn(err, res) {
    ...
    if (res.playerOne.deck + res.playerTwo.deck !== 52) {
      console.error('Play must return a total of 52 cards.')
      console.error('Response was: %s', JSON.stringify(res, null, 2))
      process.exit(2)
    }
    ...
    status(function(err, res2) {
      ...
      if (res2.playerOne + res2.playerTwo !== 52) {
        console.error('Game status must return a total of 52 cards.')
        console.error('Response was: %s', JSON.stringify(res2, null, 2))
        process.exit(3)
      }
      ...
      if (res2.playerOne > 0 && res2.playerTwo > 0) {
        ...
        console.error('Play must be 1 battle, not the entire game.')
        process.exit(4)
      } else {
        console.log('\nSuccess!');
      }
